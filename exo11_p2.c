...
{
    int matrix[LINES][COLS], number_of_zeros = -1, first_time_all_flash;
    long flashes = 0;
    ...
    for (int k = 0;; k++) {
        ...
        printf("\n");
        number_of_zeros = -1;
        first_time_all_flash = 0;

        for (int i = 0; i < LINES; i++)
            
            for (int j = 0; j < COLS; j++) {
                first_time_all_flash += matrix[i][j];
                matrix[i][j]++;
                matrix[i][j] %= 10;
        ...
        if (first_time_all_flash == 0) {
            printf("first time all octopuses flash: %d\n", k);
            break;
        }
